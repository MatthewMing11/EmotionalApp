...
def main(args):
    audio_file = args#sys.argv[1]#args
    audio_str = get_text(audio_file)
    print('=======================')
    print(audio_str)
    analyze(audio_str)
def analyze(audio_str):
    ...
    analyze.prediction_multi = model_multi.predict(np.array(X))

    X = pad_sequences(X, maxlen=20)

    analyze.prediction_posneg = model_posneg.predict(np.array(X))
    
    print(analyze.prediction_posneg[0])
    print(analyze.prediction_multi[0])
    
    out = open("out.csv",'w')
    out.write('negative,positive\n')
    out.write(','.join(map(str,analyze.prediction_posneg[0]))+'\n')
    out.write('anger,disgust,fear,guilt,joy,sadness,shame\n')
    out.write(','.join(map(str,analyze.prediction_multi[0])))
    out.close()
#main()
